...
def test(
    ...
    print(f'[test] {all_to_all_mode=}, {os.getenv("DEEP_USE_MODE")}')
    ...
    def test_func(zero_copy: bool, return_recv_hook: bool):
        # if rank == 0:
        #     print(f'[test_1] {rank=}, {current_x.shape=}, {aligned_num_tokens=}, {num_experts=}')
        ...
        # if rank == 0:
        #     print(f'[test_2] {rank=}, {simulated_gemm_x_local.shape=}, {aligned_num_tokens=}')
        # dist.barrier()
        combined_x, event, hook = buffer.low_latency_combine(
            simulated_gemm_x_local,
            topk_idx,
        ...
        # if rank == 0:
        #     print(f'[test_3] {rank=}, {combined_x.shape=}')
    ...
    simulated_gemm_x_local = (per_token_cast_back(*recv_x) if dispatch_use_fp8 else recv_x)
    combine_args = {
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="no",
        choices=["no", "int8"],
        help="Quantization type for dispatch: no (bf16), int8 (per-token)",
    )
