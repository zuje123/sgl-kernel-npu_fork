...
Two independent buffers are created (one normal, one low_latency) to avoid
resource conflicts. Both strategies share the same inputs per round, and
results are compared bitwise (normal) / per-expert (ll dispatch) + bitwise
(ll combine). A summary table is printed at the end.
"""
...
def test_compare(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    # ==========================================
    # Create buffers: one normal, one low_latency
    # ==========================================
    print(f"[Rank {rank}] Initializing normal buffer...", flush=True)
    normal_buffer = deep_ep.Buffer(
        group, int(2e9), 0, low_latency_mode=False, num_qps_per_rank=1
    )
    print(f"[Rank {rank}] Normal buffer created OK.", flush=True)
    ...
    print(f"[Rank {rank}] Low_latency buffer created OK.", flush=True)

    # Create strategy instances (bound to respective buffer runtimes)
    normal_default = DefaultNormalCommStrategy(
        runtime=normal_buffer.runtime, group=group
    )
    normal_alltoall = AlltoAllNormalCommStrategy(
        runtime=normal_buffer.runtime, group=group
    )
    DefaultLLStrategy = get_low_latency_strategy("default")
    AlltoAllLLStrategy = get_low_latency_strategy("alltoall")
    ll_default = DefaultLLStrategy(runtime=ll_buffer.runtime, group=group)
    ll_alltoall = AlltoAllLLStrategy(runtime=ll_buffer.runtime, group=group)
