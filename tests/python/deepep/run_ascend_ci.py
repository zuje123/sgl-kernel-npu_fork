...
MONITOR_POD_NAME = (
    ...
    if KUBE_JOB_TYPE == "single"
        ...
        "{}-sglang-node-0".format(os.environ.get("KUBE_JOB_NAME"))
        if KUBE_JOB_TYPE == "multi"
        else "{}-sglang-router-0".format(os.environ.get("KUBE_JOB_NAME"))
    )
)
...
if not KUBE_YAML_FILE:
    KUBE_YAML_FILE = (
        "k8s_single.yaml"
        if KUBE_JOB_TYPE == "single"
        else "k8s_multi.yaml" if KUBE_JOB_TYPE == "multi" else "k8s_pd_separation.yaml"
    )
...
def create_or_update_configmap(cm_name: str, data: dict, namespace: str):
    ...
    try:
        response = v1.create_namespaced_config_map(namespace=namespace, body=configmap)
...
def prepare_cm_data(matching_pod_string):
    pods = v1.list_namespaced_pod(namespace=KUBE_NAME_SPACE)
    cm_data = {}
...
def monitor_pod_logs(pod_name, namespace=None, timeout=None):
    ...
    pattern_lines = [
        r"\[tuning\] Dispatch \(.*\) .* GB/s \(HCCS\), .* GB/s \(RDMA\), avg_t: .* us, notify_t: .* us",
        r"\[tuning\] Combine .* GB/s \(HCCS\), .* GB/s \(RDMA\), avg_t: .* us",
    ]

    # Compile regex patterns
    patterns = [
        re.compile(line_pattern, re.IGNORECASE) for line_pattern in pattern_lines
    ]
        ...
        while process.poll() is None and not matched:
            ...
            for idx, pattern in enumerate(patterns):
                if pattern.search(line_stripped):
                    ...
                    print(f"Detected target line (type {idx+1}): {line_stripped}")

                    if matched_patterns == expected_patterns:
                        matched = True
                        print(
        ...
        if not matched:
            if process.poll() is not None:
                ...
                if stderr_output:
                    ...
                    raise Exception(
                        f"Pod logs ended but target pattern was not detected. "
                        f"Matched lines: {matched_patterns}, Expected: {expected_patterns}"
                    )
            else:
                raise Exception(
                    f"Monitoring ended but target pattern was not detected. "
                    f"Matched lines: {matched_patterns}, Expected: {expected_patterns}"
                )
...
if __name__ == "__main__":
    print(
        "Apply k8s yaml... KUBE_NAME_SPACE:{}, KUBE_CONFIG_MAP:{}, KUBE_JOB_TYPE:{}, KUBE_YAML_FILE:{}".format(
            KUBE_NAME_SPACE, KUBE_CONFIG_MAP, KUBE_JOB_TYPE, KUBE_YAML_FILE
        )
    )
