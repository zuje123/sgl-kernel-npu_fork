...
Buffer::intranode_dispatch(const torch::Tensor& x, const std::optional<torch::Tensor>& x_scales,
    ...
    auto num_experts = static_cast<int>(num_tokens_per_expert->size(0)), num_local_experts = num_experts / num_ranks;
    ...
    torch::Tensor send_data = torch::zero({num_tokens_per_rank.size(0) * 2}, dtype(torch::kInt32).device(device));
    auto num_tokens_per_expert_ptr = num_tokens_per_expert->data_ptr<int>();
    auto send_data_ptr = send_data.data_ptr<int>();
    ...
    torch::Tensor recv_data = torch::zero({num_tokens_per_rank.size(0) * 2}, dtype(torch::kInt32).device(device));
    ...
    const int num_tokens = topk_idx.size(0);
    const int num_topk = topk_idx.size(1);
    auto topk_ptr = topk_idx.data_ptr<int64_t>();
    std::vector<int32_t> local_expert_acc(num_experts, 0);

    auto send_token_idx = torch::zeros({num_tokens, num_topk}, torch::dtype(torch::kInt32).device(device));
    auto send_token_idx_ptr = send_token_idx.data_ptr<int32_t>();
    for (int i = 0; i < num_tokens; ++i) {
        for (int j = 0; j < num_topk; ++j) {
            int64_t expert_idx = topk_ptr[i * num_topk + j];
            if (expert_idx >= 0) {
    ...
    torch::Tensor send_data_offset = torch::cumsum(num_tokens_per_rank, 0);

    EP_HOST_ASSERT(recv_data.dim() == 1 and recv_data.is_contiguous());
    EP_HOST_ASSERT(recv_data.size(0) % num_experts == 0);
    torch::Tensor recv_offset = torch::zero({num_tokens_per_rank.size(0)}, dtype(torch::kInt32).device(device));
    torch::Tensor recv_count = torch::zero({num_tokens_per_rank.size(0)}, dtype(torch::kInt32).device(device));
    auto recv_data_ptr = recv_data.data_ptr<int64_t>();
    auto recv_count_ptr = recv_count.data_ptr<int64_t>();
    auto recv_offset_ptr = recv_offset.data_ptr<int64_t>();
    int64_t total_recv_tokens = 0;
    for (int64_t i = 0; i < num_tokens_per_rank.size(0); ++i) {
        recv_count_ptr[i] = recv_data_ptr[2 * i];
        recv_offset_ptr[i] = recv_data_ptr[2 * i + 1];
        total_recv_tokens += recv_count_ptr[i];
    }
    ...
    auto expandx_out = torch::zeros({total_recv_tokens, hidden}, torch::dtype(torch::kInt32).device(device));
    auto dynamic_scales_out = torch::zeros({total_recv_tokens}, torch::dtype(torch::kInt32).device(device));
    auto expand_idx_out = torch::zeros({total_recv_tokens, 3}, torch::dtype(torch::kInt32).device(device));
    ...
    auto recv_topk_idx = torch::zeros({total_recv_tokens}, torch::dtype(torch::kInt32).device(device));
    auto recv_topk_idx_ptr = recv_topk_idx.data_ptr<int64_t>();
    auto expand_idx_out_ptr = expand_idx_out.data_ptr<int64_t>();
    ...
    for (int i = 0; i < num_local_experts; ++i) {
        ...
        for (int j = 0; j < num_ranks; ++j) {
            sum += recv_count_ptr[i * num_local_experts + j];
        }
        num_recv_tokens_per_expert_list.push_back(sum);
    }

    // Wait streams
    std::optional<EventHandle> event;

    auto rank_prefix_matrix = torch::empty({num_ranks, num_ranks}, dtype(torch::kInt32).device(device));
    auto channel_prefix_matrix = torch::empty({num_ranks, num_channels}, dtype(torch::kInt32).device(device));
    auto recv_channel_prefix_matrix = torch::empty({num_ranks, num_channels}, dtype(torch::kInt32).device(device));
