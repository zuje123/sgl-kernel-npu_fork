 ...
 * Description: CamHCommMoeDistributeDispatch tiling function implementation file
 * Author: WANG Qiankun
 * Create: 2025-05-29
 * Note:
 * History: 2025-05-29 create CamHCommMoeDistributeDispatch tiling function implementation file
 */
    ...
    const bool isScales, const uint32_t quantMode)
{
    ...
    OP_TILING_CHECK(xStorageShape->GetStorageShape().GetDimNum() != TWO_DIMS,
        OP_LOGE(nodeName,
            "xShape dims must be %u, but current dim num is %lu.",
            TWO_DIMS,
    ...
    OP_TILING_CHECK(expertIdStorageShape->GetStorageShape().GetDimNum() != TWO_DIMS,
        OP_LOGE(nodeName,
            "expertIdShape dims must be %u, but current dim num is %lu.",
            TWO_DIMS,
    ...
    if (isScales) {
        ...
        OP_TILING_CHECK(scalesStorageShape->GetStorageShape().GetDimNum() != TWO_DIMS,
            OP_LOGE(nodeName,
                "scalesShape dims must be %u, but current dim num is %lu.",
                TWO_DIMS,
    ...
    OP_TILING_CHECK(expandXStorageShape->GetStorageShape().GetDimNum() != TWO_DIMS,
        OP_LOGE(nodeName,
            "expandXShape dims must be %u, but current dim num is %lu.",
            TWO_DIMS,
    ...
    OP_TILING_CHECK(expandIdxStorageShape->GetStorageShape().GetDimNum() != ONE_DIM,
        OP_LOGE(nodeName,
            "expandIdxShape dims must be %u, but current dim num is %lu.",
            ONE_DIM,
    ...
    OP_TILING_CHECK(expertTokenNumsStorageShape->GetStorageShape().GetDimNum() != ONE_DIM,
        OP_LOGE(nodeName,
            "expertTokenNumsShape dims must be %u, but current dim num is %lu.",
            ONE_DIM,
    ...
    OP_TILING_CHECK(epRecvCountStorageShape->GetStorageShape().GetDimNum() != ONE_DIM,
        OP_LOGE(nodeName,
            "epRecvCountShape dims must be %u, but current dim num is %lu.",
            ONE_DIM,
    ...
    OP_TILING_CHECK(tpRecvCountStorageShape->GetStorageShape().GetDimNum() != ONE_DIM,
        OP_LOGE(nodeName,
            "tpRecvCountShape dims must be %u, but current dim num is %lu.",
            ONE_DIM,
    ...
    const bool isScales, const uint32_t quantMode)
{
    ...
    OP_TILING_CHECK((xDesc->GetDataType() != ge::DT_BF16) && (xDesc->GetDataType() != ge::DT_FLOAT16),
        ...
        static_cast<ge::DataType>(xDesc->GetDataType())), return false);
    ...
    OP_TILING_CHECK(expertIdDesc->GetDataType() != ge::DT_INT32,
        ...
        static_cast<ge::DataType>(expertIdDesc->GetDataType())), return false);
    ...
    if (isScales) {
        ...
        OP_TILING_CHECK(scalesDesc->GetDataType() != ge::DT_FLOAT,
            ...
            static_cast<ge::DataType>(scalesDesc->GetDataType())), return false);
    }
    ...
    if (quantMode != NO_SCALES) {
        OP_TILING_CHECK(expandXDesc->GetDataType() != ge::DT_INT8,
            ...
            static_cast<ge::DataType>(expandXDesc->GetDataType())), return false);
    } else {
        OP_TILING_CHECK(expandXDesc->GetDataType() != xDesc->GetDataType(),
            ...
            static_cast<ge::DataType>(xDesc->GetDataType()), static_cast<ge::DataType>(expandXDesc->GetDataType())),
            return false);
    }
    ...
    if (quantMode == DYNAMIC_SCALES) {
        ...
        OP_TILING_CHECK(dynamicScalesDesc->GetDataType() != ge::DT_FLOAT,
            ...
            static_cast<ge::DataType>(dynamicScalesDesc->GetDataType())), return false);
    }
    ...
    OP_TILING_CHECK(expandIdxDesc->GetDataType() != ge::DT_INT32,
        ...
        static_cast<ge::DataType>(expandIdxDesc->GetDataType())), return false);
    ...
    OP_TILING_CHECK(expertTokenNumsDesc->GetDataType() != ge::DT_INT64,
        ...
        static_cast<ge::DataType>(expertTokenNumsDesc->GetDataType())), return false);
    ...
    OP_TILING_CHECK(epRecvCountsDesc->GetDataType() != ge::DT_INT32,
        ...
        static_cast<ge::DataType>(epRecvCountsDesc->GetDataType())), return false);
    ...
    OP_TILING_CHECK(tpRecvCountsDesc->GetDataType() != ge::DT_INT32,
        ...
        static_cast<ge::DataType>(tpRecvCountsDesc->GetDataType())), return false);
    return true;
}
...
static ge::graphStatus GetAttrAndSetTilingData(const gert::TilingContext &context, const char *nodeName,
    CamMoeDistributeDispatchTilingData &tilingData, std::string &groupEp, std::string &groupTp)
{
    ...
    OP_TILING_CHECK(*sharedExpertNumPtr != SHARED_EXPERT_NUM,
        OP_LOGE(nodeName,
            "sharedExpertNum only support %u, but got sharedExpertNum=%ld.",
            SHARED_EXPERT_NUM,
            *sharedExpertNumPtr),
        return ge::GRAPH_FAILED);
    ...
    CamMoeDistributeDispatchTilingData &tilingData, uint32_t &localMoeExpertNum)
{
    ...
    OP_TILING_CHECK((tpWorldSize > 1) && (localMoeExpertNum > 1),
        OP_LOGE(nodeName, "Cannot support multi-moeExpert %u "
    ...
    OP_TILING_CHECK(epWorldSize % 8 != 0, OP_LOGE(nodeName,
        "epWorldSize should be divisible by 8, but got epWorldSize = %u.", epWorldSize), return ge::GRAPH_FAILED);
...
static ge::graphStatus CheckTensorShape(const gert::TilingContext &context, const char *nodeName,
    CamMoeDistributeDispatchTilingData &tilingData, const uint32_t quantMode, const bool isScales,
    const bool isSharedExpert, const int64_t localMoeExpertNum)
{
    ...
    OP_TILING_CHECK((xDim1 != SUPPORT_HIDDEN_SIZE), OP_LOGE(nodeName, "xShape dims1(H) only supports %u, but got %ld.",
        SUPPORT_HIDDEN_SIZE, xDim1),
        return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(expertIdStorageShape == nullptr,
        OP_LOGE(nodeName, "expertIdStorageShape is nullptr."),
        return ge::GRAPH_FAILED);
    ...
    if (isScales) {
        ...
        OP_TILING_CHECK(scalesStorageShape == nullptr,
            OP_LOGE(nodeName, "scalesStorageShape is nullptr."),
            return ge::GRAPH_FAILED);
        ...
        if (sharedExpertRankNum == 0U) {
            ...
            OP_TILING_CHECK(scalesDim0 != (moeExpertNum + 1UL), OP_LOGE(nodeName,
                "scales's dim0 not equal to moeExpertNum + 1, scales's dim0 is %ld, moeExpertNum + 1 is %ld.",
                scalesDim0, moeExpertNum + 1UL), return ge::GRAPH_FAILED);
        }
    ...
    OP_TILING_CHECK(expandXStorageShape == nullptr,
        OP_LOGE(nodeName, "expandXStorageShape is nullptr."),
        return ge::GRAPH_FAILED);
    ...
    if (quantMode != NO_SCALES) {
        const gert::StorageShape *dynamicScalesStorageShape = context.GetOutputShape(OUTPUT_DYNAMIC_SCALES_INDEX);
        OP_TILING_CHECK(dynamicScalesStorageShape == nullptr,
            OP_LOGE(nodeName, "dynamicScalesStorageShape is nullptr."),
            return ge::GRAPH_FAILED);
    ...
    // 校验expandIdx的维度
    const gert::StorageShape *expandIdxStorageShape = context.GetOutputShape(OUTPUT_EXPAND_IDX_INDEX);
    OP_TILING_CHECK(expandIdxStorageShape == nullptr,
        OP_LOGE(nodeName, "expandIdxStorageShape is nullptr."),
        return ge::GRAPH_FAILED);
    ...
    const gert::StorageShape *expertTokenNumsStorageShape = context.GetOutputShape(OUTPUT_EXPERT_TOKEN_NUMS_INDEX);
    OP_TILING_CHECK(expertTokenNumsStorageShape == nullptr,
        OP_LOGE(nodeName, "expertTokenNumsStorageShape is nullptr."),
        return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(epRecvCountStorageShape == nullptr,
        OP_LOGE(nodeName, "epRecvCountStorageShape is nullptr."),
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(tpRecvCountStorageShape == nullptr,
        OP_LOGE(nodeName, "tpRecvCountStorageShape is nullptr."),
        return ge::GRAPH_FAILED);
...
static ge::graphStatus MoeDistributeDispatchA2CheckShapeAndSetTiling(
    const gert::TilingContext &context, CamMoeDistributeDispatchA2Info& info)
{
    const char *nodeName = context.GetNodeName();
    ...
    const gert::StorageShape *srcOffsetRankTokenIdxStorageShape = context.GetInputShape(SRC_OFFSET_RANK_TOKEN_IDX_INDEX);
    ...
    const gert::StorageShape *dstOffsetRankTokenIdxStorageShape = context.GetInputShape(DST_OFFSET_RANK_TOKEN_IDX_INDEX);
    ...
    const gert::TilingContext &context, CamMoeDistributeDispatchA2Info& info)
{
    ...
    OP_TILING_CHECK(moeExpertNumPtr == nullptr,
        OP_LOGE(K_INNER_DEBUG, "moeExpertNumPtr is null."), return GRAPH_FAILED);
    OP_TILING_CHECK(*moeExpertNumPtr % *epWorldSizePtr != 0 ||
        *moeExpertNumPtr <= 0 || *moeExpertNumPtr > MAX_MOE_EXPERT_NUMS_A2,
        ...
        MAX_MOE_EXPERT_NUMS_A2, *moeExpertNumPtr), return GRAPH_FAILED);
...
{
    OP_LOGI(nodeName, "start MoeDistributeDispatchA2TilingFuncImpl func.");
    const char *nodeName = context.GetNodeName();
    ...
    // 3. communication
    auto attrs = context.GetAttrs();
    OP_TILING_CHECK(attrs == nullptr, OP_LOGE(nodeName, "attrs is nullptr."),
        return ge::GRAPH_FAILED);
...
IMPL_OP_OPTILING(CamHCommMoeDistributeDispatch)
