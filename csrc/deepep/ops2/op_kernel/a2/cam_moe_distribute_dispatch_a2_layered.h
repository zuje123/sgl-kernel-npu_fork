...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR expertIds, GM_ADDR scales, GM_ADDR expertScales, GM_ADDR tokenServerIdx,
...
__aicore__ inline void CamMoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::Init(GM_ADDR x,
    GM_ADDR expertIds, GM_ADDR scales, GM_ADDR expertScales, GM_ADDR tokenServerIdx,
    GM_ADDR tokenServerCnt, GM_ADDR epRankTokenCnt, GM_ADDR srcOffsetRankTokenIdx, GM_ADDR dstOffsetRankTokenIdx, 
    GM_ADDR expandXOut, GM_ADDR dynamicScalesOut, GM_ADDR expandIdxOut, GM_ADDR expertTokenNumsOut, GM_ADDR epRecvCountsOut,
    GM_ADDR expandScales, GM_ADDR workspaceGM, TPipe *pipe, GM_ADDR tilingGM)
{
...
__aicore__ inline void CamMoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::Input2Win()
{
    ...
    DataCopyExtParams tokenServerIdxParams = {
        1U, static_cast<uint32_t>(sendTokenNum * serverNum * sizeof(int32_t)), 0U, 0U, 0U};
    DataCopyPadExtParams<int32_t> copyPadExtParams{false, 0U, 0U, 0U};
    DataCopyPad(tokenServerIdxTensor_,
        tokenServerIdxGMTensor_[startTokenId * serverNum],
    ...
    DataCopyExtParams tokenCopyParamsNoQuant{
        static_cast<uint16_t>(1), static_cast<uint16_t>(tokenLenInStruct_), 0, 0, 0};
...
__aicore__ inline void CamMoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::Ipc2Out()
{
    ...
    for (uint32_t srcRank = 0; srcRank < worldSize_; ++srcRank) {
        ...
        uint32_t targetRankId = localRankIdx + curServerIdx * SERVER_RANK_SIZE;  // 4+1*8=12 当前server上的rank，全局rankid=12
        ...
        srcIpcGt.SetGlobalBuffer(
            (__gm__ uint8_t *)(shareAddrWins[localRankIdx]) + tarServerBlockIdx * SERVER_SIZE_ON_WIN + TOKEN_COUNT_SIZE);

        for (uint32_t recvExpId = curRankExpertStart; recvExpId <= curRankExpertEnd; ++recvExpId) {
            int recvTokenCnt = epRankTokenCntGMTensor_.GetValue(
                recvExpId * worldSize_ + srcRank);  // 专家recvExpId从srcRank收的token个数
            ...
            LocalTensor<uint8_t> localUB =
                tBuf.GetWithOffset<uint8_t>((DISPATCH_TOKEN_UB_SIZE - TBUF_TEMP_OFFSET) / sizeof(uint8_t), TBUF_TEMP_OFFSET);
            ...
            for (int i = beginIndex; i < endIndex; ++i) {
                ...
                uint32_t tokenOffset = (tokenStructLen_ * srcOffset); // 包含token, 以及token后的信息:expIds, weights, tokenIdx, scales
