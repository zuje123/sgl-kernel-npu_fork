...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR expertIds, GM_ADDR scales, GM_ADDR expertScales, GM_ADDR tokenMap,
        GM_ADDR tokenServerIdx, GM_ADDR tokenServerCnt, GM_ADDR expandXOut, GM_ADDR dynamicScalesOut,
...
private:
    ...
    GlobalTensor<int32_t> tokenMapGMTensor_;
    ...
    LocalTensor<int32_t> tokenMapTensor_;
...
template <TemplateMC2TypeA2layeredClass>
__aicore__ inline void CamMoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::Init(GM_ADDR x,
    GM_ADDR expertIds, GM_ADDR scales, GM_ADDR expertScales, GM_ADDR tokenMap, GM_ADDR tokenServerIdx,
    GM_ADDR tokenServerCnt, GM_ADDR expandXOut, GM_ADDR dynamicScalesOut, GM_ADDR expandIdxOut, GM_ADDR expertTokenNumsOut,
...
*/
    ...
    // struce相关信息初始化计算
    tokenStructLen_ = axisH_ * sizeof(ExpandXOutType) + INFO_NUM_IN_TOKENSTRUCK * (kAlign_ * sizeof(uint32_t)); // token和四元组大小
    ...
    scaleOffsetInStruct_ = tokenLenInStruct_ + expLenInStruct_ + weightLenInStruct_ + cntLenInStruct_; // 开始写scales的起始位置
    ...
    tokenMapGMTensor_.SetGlobalBuffer((__gm__ int32_t *)tokenMap);
    ...
    dataBatchWriteInfoTensor_.SetGlobalBuffer(
        (__gm__ uint64_t *)(dataBatchWriteInfo_), serverNum * PER_MSG_RDMA_SEND_TIME * B64_PER_BLOCK);

    expertToServerCntGM_ = dataBatchWriteInfo_ + serverNum * PER_MSG_RDMA_SEND_TIME * B64_PER_BLOCK * sizeof(uint64_t);
    expertToServerGlobalTensor_.SetGlobalBuffer(
        (__gm__ uint32_t *)(expertToServerCntGM_), RoundUp(axisBS_ * serverNum, B32_PER_BLOCK));
    ...
    tpipe_->InitBuffer(expertIdsBuf_,
        RoundUp(axisBS_ * kAlign_ * sizeof(int32_t), (unsigned long)B32_PER_BLOCK));  // bs * k * 4 = 125K
    expertIdsTensor_ = expertIdsBuf_.Get<int32_t>();
    */

    tpipe_->InitBuffer(expertToServerCntsBuf_,
        RoundUp(static_cast<uint32_t>(axisBS_ * serverNum * sizeof(uint32_t)), UB_32B_ALIGN));  // bs * rankSize / 8 * 4
    expertToServerCntTensor_ = expertToServerCntsBuf_.Get<uint32_t>();
    Duplicate<uint32_t>(expertToServerCntTensor_,
        0,
...
template <TemplateMC2TypeA2layeredClass>
__aicore__ inline void CamMoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::ReorderTokens()
{
    ...
    tpipe_->InitBuffer(tokenMapBuf_, sendTokenNum * sizeof(int32_t));
    tpipe_->InitBuffer(tokenServerIdxBuf_, sendTokenNum * sizeof(int32_t));
    tpipe_->InitBuffer(serverCountBuf_, serverNum * sizeof(int32_t));
    tokenMapTensor_ = tokenMapBuf_.Get<int32_t>();
    tokenServerIdxTensor_ = tokenServerIdxBuf_.Get<int32_t>();
    serverCountTensor_ = serverCountBuf_.Get<int32_t>();
    DataCopyExtParams tokenMapParams = {
        1U, static_cast<uint32_t>(sendTokenNum * serverNum * sizeof(int32_t)), 0U, 0U, 0U};
    DataCopyExtParams serverCountParams = {1U, static_cast<uint32_t>(serverNum * sizeof(int32_t)), 0U, 0U, 0U};
    DataCopyPadExtParams<int32_t> copyPadExtParams{false, 0U, 0U, 0U};
    DataCopyPad(tokenMapTensor_, tokenMapGMTensor_[startTokenId * serverNum], tokenMapParams, copyPadExtParams);
    DataCopyPad(
        tokenServerIdxTensor_, tokenServerIdxGMTensor_[startTokenId * serverNum], tokenMapParams, copyPadExtParams);
    ...
    LocalTensor<XType> tokenLt =
        tBuf.GetWithOffset<XType>(((tokenUbSize) / sizeof(XType)), TBUF_TEMP_OFFSET);
    ...
    */
    ...
    DataCopyExtParams tokenCopyParamsNoQuant{static_cast<uint16_t>(1),
        static_cast<uint16_t>(tokenLenInStruct_),
        0,
        static_cast<uint16_t>(tokenGapInStruct_),
        0};
    DataCopyPadExtParams<uint8_t> tokenPadParams;

    DataCopyExtParams expCopyParams{static_cast<uint16_t>(1),
        static_cast<uint16_t>(realLenInStruct_),
        0,
        static_cast<uint16_t>(infoGapInStruct_),
        0};
    DataCopyPadExtParams<uint8_t> expPadParams;

    DataCopyExtParams weightCopyParams{static_cast<uint16_t>(1),
        static_cast<uint16_t>(realLenInStruct_),
        0,
        static_cast<uint16_t>(infoGapInStruct_),
        0};
    ...
    for (int i = 0; i < sendTokenNum; i++) {
        if constexpr (DynamicQuant || StaticQuant) {
            DataCopyPad(tokenTempTensorU8_,
                xGMTensorU8_[(startTokenId + i) * axisH_ * sizeof(XType)],
                tokenCopyParamsQuant,
                tokenPadParams);
            LocalTensor<float> tokenCastLt =
                tBuf.GetWithOffset<float>(((axisH_ * sizeof(float)) / sizeof(float)),
                    RoundUp(TBUF_TEMP_OFFSET + tokenUbSize, B32_PER_BLOCK));
            QuantProcess(1, tokenLt, tokenCastLt);
        } else {
            DataCopyPad(tokenTempTensorU8_,
                xGMTensorU8_[(startTokenId + i) * tokenLenInStruct_],
                tokenCopyParamsNoQuant,
                tokenPadParams);
        }
        // 拷贝topkIds 可省略
        DataCopyPad(tokenTempTensorU8_[expOffsetInStruct_],
            expertIdsGMTensorU8_[(startTokenId + i) * realLenInStruct_],
            expCopyParams,
            expPadParams);
        // 拷贝weight
        DataCopyPad(tokenTempTensorU8_[weightOffsetInStruct_],
            weightGt[(startTokenId + i) * realLenInStruct_],
            weightCopyParams,
            weightPadParams);
        SyncFunc<AscendC::HardEvent::MTE2_S>();
        for (int j = 0; j < serverNum; j++) {
            if (tokenMapTensor_[i * serverNum + j] == 0) {
                continue;
            }
            uint32_t destOffset = j * SERVER_SIZE_ON_WIN +
                                  tokenStructLen_ * tokenServerIdxTensor_[i * serverNum + j] + TOKEN_COUNT_SIZE;
            DataCopy(
                sendTokensU8Tensor_[destOffset], tokenTempTensorU8_[0], tokenStructLen_);
        }
...
template <TemplateMC2TypeA2layeredClass>
__aicore__ inline void CamMoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::GatherAndWriteCntInfo()
{
    ...
    if ASCEND_IS_AIV {
        if (aivId_ == 0) {
            HcclHandle batchWriteResultData = hccl_.BatchWrite<true>(
                (GM_ADDR)(dataBatchWriteInfoTensor_.GetPhyAddr()), serverNum * PER_MSG_RDMA_SEND_TIME);
...
template <TemplateMC2TypeA2layeredClass>
__aicore__ inline uint32_t CamMoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::GetExpRank(
    uint32_t expertId)
{
...
template <TemplateMC2TypeA2layeredClass>
__aicore__ inline bool CamMoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::IsInSameServer(
...
template <TemplateMC2TypeA2layeredClass>
__aicore__ inline int64_t CamMoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::MergeMagicWithValue(
    int32_t magic, int32_t value)
{
...
template <TemplateMC2TypeA2layeredClass>
__aicore__ inline void CamMoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::SetTokenCnt(
...
__aicore__ inline void CamMoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::Win2Ipc()
{
    ...
    for (uint32_t batchIdx = 0; batchIdx < batchCnt; ++batchIdx) {
        ...
        if (formServerId == rankId_ / SERVER_RANK_SIZE) {
            ...
            DataCopy(localUB,
                sendTokensU8Tensor_[WinInTokenOffset + tokenStruceIdx * tokenStructLen_],
                tokenCntInBatch * tokenStructLen_);
        } else {
            SyncFunc<AscendC::HardEvent::S_MTE2>();
            DataCopy(localUB,
                readTokensU8Tensor_[WinInTokenOffset + tokenStruceIdx * tokenStructLen_],
                tokenCntInBatch * tokenStructLen_);
        }
        SyncFunc<AscendC::HardEvent::MTE2_S>();
...
__aicore__ inline void CamMoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::Ipc2Out()
{
    ...
    DataCopyExtParams copyExpertIdsParams{1,
        static_cast<uint32_t>(
            serverNum * SERVER_RANK_SIZE * localMoeExpertNum_ * EXP_TOKEN_COUNT_FLAG_CNT * sizeof(int32_t)),
        0,
        0,
        0};
    DataCopyPadExtParams<int32_t> padParams;
...
#pragma unroll 8
    for (uint32_t i = 0; i < tempSize; ++i) {  // 当前核读取的token个数
    ...
    LocalTensor<uint8_t> localUB =
        tBuf.GetWithOffset<uint8_t>(DISPATCH_TOKEN_UB_SIZE / sizeof(uint8_t), TBUF_TEMP_OFFSET);
    LocalTensor<float> localUBfloat =
        tBuf.GetWithOffset<float>(DISPATCH_TOKEN_UB_SIZE / sizeof(float), TBUF_TEMP_OFFSET);
    LocalTensor<int32_t> localUBint32 =
        tBuf.GetWithOffset<int32_t>(DISPATCH_TOKEN_UB_SIZE / sizeof(int32_t), TBUF_TEMP_OFFSET);
    ...
    for (uint32_t idx = 0; idx < srCntCurCore; ++idx) {
        ...
        for (uint32_t batchIdx = 0; batchIdx < batchCnt; ++batchIdx) {
            ...
            DataCopyExtParams copyTokenParams{
                static_cast<uint16_t>(1), static_cast<uint32_t>(tokenCntInBatch * tokenStructLen_), 0, 0, 0};
            ...
            DataCopyExtParams writeTokenParams{static_cast<uint16_t>(tokenCntInBatch),
                ...
                static_cast<uint32_t>(tokenGapInStruct_),
                0,
                0};
            ...
            // weight output
            PipeBarrier<PIPE_ALL>();
            DataCopyExtParams weightTokenParams{
                static_cast<uint16_t>(1), static_cast<uint32_t>(tokenCntInBatch * sizeof(float)), 0, 0, 0};
            ...
            if constexpr (DynamicQuant) {
                DataCopyExtParams quantTokenParams{static_cast<uint16_t>(tokenCntInBatch),
                    ...
                    static_cast<uint32_t>((tokenStructLen_ - UB_32B_ALIGN) / UB_32B_ALIGN),
                    0,
                    0};

                LocalTensor<float> quantTempUB = localUB[scaleOffsetInStruct_].ReinterpretCast<float>();
                DataCopyPad(
                    dynamicScalesOutGMTensor_[(sumTokenCnt + batchIdx * tokenCntInUB)], quantTempUB, quantTokenParams);
            }
            SyncFunc<AscendC::HardEvent::MTE3_MTE2>();
        }
