...
#include "../kernels/moe_dispatch_tiling.h"
...
namespace optiling {
static void PrintTilingDataInfo(const char *nodeName, MoeDispatchTilingData &tilingData)
{
    OP_LOGD(nodeName, "epWorldSize is %u.", tilingData.moeDispatchInfo.epWorldSize);
    OP_LOGD(nodeName, "tpWorldSize is %u.", tilingData.moeDispatchInfo.tpWorldSize);
    OP_LOGD(nodeName, "epRankId is %u.", tilingData.moeDispatchInfo.epRankId);
    OP_LOGD(nodeName, "tpRankId is %u.", tilingData.moeDispatchInfo.tpRankId);
    OP_LOGD(nodeName, "moeExpertNum is %u.", tilingData.moeDispatchInfo.moeExpertNum);
    OP_LOGD(nodeName, "quantMode is %u.", tilingData.moeDispatchInfo.quantMode);
    OP_LOGD(nodeName, "globalBs is %u.", tilingData.moeDispatchInfo.globalBs);
    OP_LOGD(nodeName, "bs is %u.", tilingData.moeDispatchInfo.bs);
    OP_LOGD(nodeName, "k is %u.", tilingData.moeDispatchInfo.k);
    OP_LOGD(nodeName, "h is %u.", tilingData.moeDispatchInfo.h);
    OP_LOGD(nodeName, "aivNum is %u.", tilingData.moeDispatchInfo.aivNum);
    OP_LOGD(nodeName, "totalUbSize is %lu.", tilingData.moeDispatchInfo.totalUbSize);
    OP_LOGD(nodeName, "totalWinSize is %lu.", tilingData.moeDispatchInfo.totalWinSize);
}
...
static ge::graphStatus GetAttrAndSetTilingData(gert::TilingContext *context, const char *nodeName,
    MoeDispatchTilingData &tilingData, std::string &groupEp, std::string &groupTp)
{
    ...
    tilingData.moeDispatchInfo.epWorldSize = static_cast<uint32_t>(epWorldSize);
    tilingData.moeDispatchInfo.tpWorldSize = static_cast<uint32_t>(*tpWorldSizePtr);
    tilingData.moeDispatchInfo.epRankId = static_cast<uint32_t>(*epRankIdPtr);
    tilingData.moeDispatchInfo.tpRankId = static_cast<uint32_t>(*tpRankIdPtr);
    tilingData.moeDispatchInfo.moeExpertNum = static_cast<uint32_t>(moeExpertNum);
    tilingData.moeDispatchInfo.quantMode = static_cast<uint32_t>(*quantModePtr);

    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus CheckAttrs(
    gert::TilingContext *context, const char *nodeName, MoeDispatchTilingData &tilingData, uint32_t &localMoeExpertNum)
{
    uint32_t epWorldSize = tilingData.moeDispatchInfo.epWorldSize;
    uint32_t tpWorldSize = tilingData.moeDispatchInfo.tpWorldSize;
    uint32_t moeExpertNum = tilingData.moeDispatchInfo.moeExpertNum;
    ...
    OP_LOGD(nodeName, "MoeDispatch *globalBsPtr = %ld, bs = %ld, epWorldSize = %u\n", *globalBsPtr, xDim0, epWorldSize);
    ...
    tilingData.moeDispatchInfo.globalBs = static_cast<uint32_t>(*globalBsPtr);

    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus CheckTensorShape(gert::TilingContext *context, const char *nodeName,
    MoeDispatchTilingData &tilingData, const uint32_t quantMode, const int64_t localMoeExpertNum)
{
    uint32_t A = 0U;
    uint32_t globalBs = tilingData.moeDispatchInfo.globalBs;
    ...
    tilingData.moeDispatchInfo.h = static_cast<uint32_t>(xDim1);

    // 校验expert_id的维度并设k
    int64_t moeExpertNum = static_cast<int64_t>(tilingData.moeDispatchInfo.moeExpertNum);
    const gert::StorageShape *expertIdStorageShape = context->GetInputShape(EXPERT_IDS_INDEX);
    ...
    OP_TILING_CHECK((expertIdsDim1 <= 0) || (expertIdsDim1 > K_MAX) || (expertIdsDim1 > moeExpertNum),
        OP_LOGE(nodeName,
            ...
            expertIdsDim1),
        return ge::GRAPH_FAILED);
    tilingData.moeDispatchInfo.k = static_cast<uint32_t>(expertIdsDim1);
...
static ge::graphStatus TilingCheckMoeDispatch(
...
static void SetHcommCfg(const gert::TilingContext *context, MoeDispatchTilingData *tiling, const std::string groupEp,
    const std::string groupTp)
{
    const char *nodeName = context->GetNodeName();
    OP_LOGD(nodeName, "MoeDispatch groupEp = %s, groupTp = %s", groupEp.c_str(), groupTp.c_str());
...
static ge::graphStatus MoeDispatchA3TilingFuncImpl(gert::TilingContext *context)
{
    const char *nodeName = context->GetNodeName();
    MoeDispatchTilingData *tilingData = context->GetTilingData<MoeDispatchTilingData>();
    ...
    quantMode = tilingData->moeDispatchInfo.quantMode;

    // 检查输入输出的dim、format、dataType
    OP_TILING_CHECK(TilingCheckMoeDispatch(context, nodeName, quantMode) != ge::GRAPH_SUCCESS,
    ...
    uint64_t h = static_cast<uint64_t>(tilingData->moeDispatchInfo.h);
    uint64_t k = static_cast<uint64_t>(tilingData->moeDispatchInfo.k);
    uint64_t epWorldSize = static_cast<uint64_t>(tilingData->moeDispatchInfo.epWorldSize);
    uint64_t maxBs = static_cast<uint64_t>(tilingData->moeDispatchInfo.globalBs) / epWorldSize;
    ...
    OP_TILING_CHECK((actualSize > maxWindowSize),
        ...
        return ge::GRAPH_FAILED);
    tilingData->moeDispatchInfo.totalWinSize = maxWindowSize;
    ...
    tilingData->moeDispatchInfo.totalUbSize = ubSize;
    tilingData->moeDispatchInfo.aivNum = aivNum;
    OP_LOGD(nodeName, "blockDim=%u, aivNum=%u, ubSize=%lu", blockDim, aivNum, ubSize);
    PrintTilingDataInfo(nodeName, *tilingData);
    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus MoeDispatchTilingFunc(gert::TilingContext *context)
{
    ge::graphStatus ret = MoeDispatchA3TilingFuncImpl(context);
    return ret;
}

struct MoeDispatchCompileInfo {};
ge::graphStatus TilingParseForMoeDispatch(gert::TilingParseContext *context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(MoeDispatch)
    .Tiling(MoeDispatchTilingFunc)
    .TilingParse<MoeDispatchCompileInfo>(TilingParseForMoeDispatch);
}  // namespace optiling