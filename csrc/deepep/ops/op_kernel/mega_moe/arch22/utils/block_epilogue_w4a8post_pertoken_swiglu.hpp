...
/*!
 * \file block_epilogue_w4a8post_pertoken_swiglu.hpp
 * \brief
 */
...
public:
    ...
    struct Params {
        ...
        Params(__gm__ ElementPerTokenScale *ptrPerTokenScale_, LayoutPerTokenScale const &layoutPerTokenScale_,
            ...
            : ptrPerTokenScale(ptrPerTokenScale_), layoutPerTokenScale(layoutPerTokenScale_), ptrD(ptrD_),
              layoutD(layoutD_), expertPerRank(expertPerRank_)
        {
        }
    };
                    ...
                    uint32_t gmmOutPreRowStride = 1, Callback &&callback = Callback{})
    {
        ...
        for (uint32_t loopIdx = loopStartIdx; loopIdx < loopStartIdx + tasksForIdx; ++loopIdx) {
            ...
            LayoutC layoutGmC{2, blockN, gmmOutPreRowStride};
            ...
            if (loopIdx < loopStartIdx + UB_STAGES) // Initialize auxiliary matrix for the first time
            {
                ...
                DataCopyPad(ubweighAux, weightAux[gmOffsetwA], copyParams, padParams);
